        ...
        for (GbDepartmentOrdersEntity orders : ordersEntities) {
            ...
            if (orders.getGbDoNxDepartmentOrderId() != null && orders.getGbDoNxDepartmentOrderId() != -1) {
                ...
                if (ordersEntity1 != null) {
                    ...
                    NxDepartmentDisGoodsEntity departmentDisGoodsEntity = nxDepartmentDisGoodsService.queryDepartmentGoods(mapGbDep);
...
//            BigDecimal multiply = new BigDecimal(total).divide(new BigDecimal(integer2),1,BigDecimal.ROUND_HALF_UP);
    ...
    public R getGbPurGoodsFenxi(Integer disGoodsId, String startDate, String stopDate) {
        ...
        if (integer > 0) {
            ...
            System.out.println("whwkkwkwwkwkwkkwkwkwwwkwk");
            Map<String, Object> mapEveryDay = new HashMap<>();
...
//            System.out.println("whserereiieieieiieimapreekv"+ goodsEntity.get);
            System.out.println("whserereiieieieiieimapreekv" + mapEveryDay);
     ...
     * 计算日期范围内的天数
     */
    private Integer calculateDaysInPeriod(String startDate, String stopDate) {
        if (!startDate.equals(stopDate)) {
            return getHowManyDaysInPeriod(stopDate, startDate);
        }
        return 0;
    }
     ...
     * 检查是否有采购数据
     */
    private boolean hasPurchaseData(Integer disGoodsId, String startDate, String stopDate) {
        Map<String, Object> map1 = new HashMap<>();
        map1.put("dayuStatus", 1);
        map1.put("disGoodsId", disGoodsId);
        map1.put("startDate", startDate);
     ...
     * 获取采购统计数据
     */
    private PurchaseStats getPurchaseStatistics(Integer disGoodsId, String startDate, String stopDate) {
        Map<String, Object> map = new HashMap<>();
        map.put("startDate", startDate);
        map.put("stopDate", stopDate);
        map.put("disGoodsId", disGoodsId);
        ...
        int purCount = gbDpgService.queryGbPurchaseGoodsCount(map);
        double doubleCost = 0;
        double doubleCostV = 0;
        String maxPrice = "0";
        String minPrice = "0";
        ...
        if (purCount > 0) {
            ...
            doubleCost = gbDpgService.queryPurchaseGoodsWeightTotal(map);
            System.out.println("AAAAA" + doubleCostV + "bbb" + doubleCost);
            double v = doubleCostV / doubleCost;
            perPrice = new BigDecimal(v).setScale(1, BigDecimal.ROUND_HALF_UP).toString();
            maxPrice = gbDpgService.queryPurGoodsMaxPrice(map);
            minPrice = gbDpgService.queryPurGoodsMinPrice(map);
        }

        return new PurchaseStats(doubleCost, doubleCostV, maxPrice, minPrice, perPrice, purCount);
    }
