package com.nongxinle.platform.admin.controller;
...
public class PlatformCouponAdminController {
    ...
    @RequestMapping(value = "/nx-goods/category/options", method = RequestMethod.POST)
    @ResponseBody
    public R listNxGoodsCategoryOptions() {
        try {
            return R.ok("success").put("data", platformCouponAdminService.listNxGoodsCategoryOptions());
        } catch (IllegalArgumentException | IllegalStateException e) {
            return R.error(-1, e.getMessage());
        } catch (Exception e) {
            log.error("[platform/admin/coupon/nx-goods/category/options] failed", e);
            return R.error("查询 nx_goods 分类失败");
        }
    }

    @RequestMapping(value = "/store/options", method = RequestMethod.POST)
    @ResponseBody
    public R listStoreOptions() {
        try {
            return R.ok("success").put("data", platformCouponAdminService.listStoreOptions());
        } catch (IllegalArgumentException | IllegalStateException e) {
            return R.error(-1, e.getMessage());
        } catch (Exception e) {
            log.error("[platform/admin/coupon/store/options] failed", e);
            return R.error("查询门店列表失败");
        }
    }
