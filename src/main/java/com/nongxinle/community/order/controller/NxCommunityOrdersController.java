package com.nongxinle.community.order.controller;
...
public class NxCommunityOrdersController {
    ...
    public R customerFetchOrder(@RequestBody NxCommunityOrdersEntity orders) {
        ...
        verifyUserCouponOnOrderPaid(orders);
    ...
    public R customerIndexData(Integer commId, Integer orderUserId) {
        ...
        mapC.put("status", 0);
        mapC.put("publicActiveOnly", true);
...
//        int cardCount = nxCustomerUserCardService.queryUserCardCount(mapCARD2);
    ...
    public R deleteOrder(@PathVariable Integer nxOrdersId) {
        ...
        NxCommunityOrdersEntity order = nxCommunityOrdersService.queryObject(nxOrdersId);
        ...
        releaseUserCouponOnOrderCancel(order);
        ...
        // 4. 判断是否需要发起微信退款：只有支付成功（payment_status == 1）且存在微信交易号时才退款
        Integer paymentStatus = nxCommunityOrdersEntity.getNxCoPaymentStatus();
        String outTradeNo = nxCommunityOrdersEntity.getNxCoWxOutTradeNo();

        // payment_status != 1 或 没有微信交易号 → 未实际支付，跳过退款
        if (paymentStatus == null || paymentStatus != 1 || outTradeNo == null || outTradeNo.trim().isEmpty()) {
            System.out.println("订单未支付（paymentStatus=" + paymentStatus + "），无需微信退款，直接删除。");
            return R.ok();
        }
        ...
        reqJson.put("notify_url", "https://grainservice.club:8443/nongxinle/api/nxorders/refund");
        ...
        double doubleVal = Double.parseDouble(refundAmountString);
        int totalFen = (int) Math.round(doubleVal * 100);
        JSONObject amount = new JSONObject();
        amount.put("total", totalFen);
        amount.put("refund", totalFen);  // 全额退款
...
//        nxCommunityOrdersService.justSaveWithUserGoods(nxOrders);
    ...
    public R applyCoupon(@RequestBody MiniappApplyCouponRequest request) {
        try {
            return R.ok().put("data", couponApplyService.applyToMiniappOrder(request));
    ...
    @ResponseBody
    @RequestMapping(value = "/releaseOrderCoupon", method = RequestMethod.POST)
    public R releaseOrderCoupon(Integer orderId, Integer customerUserId) {
        try {
            couponApplyService.unlockLockedCouponForOrder(orderId, customerUserId);
            return R.ok();
    ...
    public String callBack(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            if (notifyMap.get("result_code").equals("SUCCESS")) {
                ...
                couponApplyService.verifyCouponOnPaymentSuccess(billEntity);
                couponApplyService.verifyCouponOnPaymentSuccess(billEntity);
...
//        content += "--------------------------------<BR>";
    ...
    private void verifyUserCouponOnOrderPaid(NxCommunityOrdersEntity order) {
        if (order.getNxCoUserCouponId() == null) {
            return;
        }
        NxCustomerUserCouponEntity customerUserCouponEntity =
                nxCustomerUserCouponService.getUserCouponById(order.getNxCoUserCouponId());
        if (customerUserCouponEntity == null) {
            return;
        }
    ...
    private void releaseUserCouponOnOrderCancel(NxCommunityOrdersEntity order) {
