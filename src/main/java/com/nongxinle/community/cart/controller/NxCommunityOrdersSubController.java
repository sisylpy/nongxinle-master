...
package com.nongxinle.community.cart.controller;
...
import com.nongxinle.community.order.splicing.service.NxCommunitySplicingOrdersService;
...
import com.nongxinle.community.cart.helper.MiniappCartPricingHelper;
...
public class NxCommunityOrdersSubController {
    ...
    public R editOrderSubOrders(Integer orderType, Integer nxOrdersId) {
        couponApplyService.unlockLockedCouponForOrder(nxOrdersId, null);
        if (orderType == 1) {
            ...
            if (subEntities.size() > 0) {
                for (NxCommunityOrdersSubEntity subEntity : subEntities) {
                    ...
                    if (sameGoodsOrder != null) {
                        ...
                        BigDecimal decimal = new BigDecimal(sameGoodsOrder.getNxCosPrice()).multiply(add).setScale(2, BigDecimal.ROUND_HALF_UP);
...
//                        BigDecimal orderQuantity = new BigDecimal(subEntity.getNxCosQuantity());
                        ...
                        BigDecimal decimal = new BigDecimal(goodsEntity.getNxCgGoodsHuaxianPrice()).multiply(orderQuantity).setScale(2, BigDecimal.ROUND_HALF_UP);
                ...
                for (NxCommunityOrdersSubEntity subEntity : subEntities) {
                    ...
                    if (goodsEntity.getNxCgGoodsHuaxianPrice() != null) {
                        ...
                        if (moreQuantity.compareTo(new BigDecimal(0)) == 1) {
                            ...
                            BigDecimal decimal = new BigDecimal(goodsEntity.getNxCgGoodsPrice()).multiply(orderQuantity).setScale(2, BigDecimal.ROUND_HALF_UP);
                            ...
                            if (sameGoodsOrder != null) {
                                ...
                                if (moreQuantity.compareTo(new BigDecimal(0)) == 0) {
                                    ...
                                    BigDecimal decimal = new BigDecimal(goodsEntity.getNxCgGoodsPrice()).multiply(new BigDecimal(goodsEntity.getNxCgGoodsHuaxianQuantity())).setScale(2, BigDecimal.ROUND_HALF_UP);
                                    ...
                                    BigDecimal decimal1 = restQuantity.multiply(new BigDecimal(goodsEntity.getNxCgGoodsHuaxianPrice())).setScale(2, BigDecimal.ROUND_HALF_UP);
...
//    }
    ...
    public R orderUserGetApply(Integer userId, Integer serviceType, Integer addressId) {
        ...
        Integer communityId = resolveCommunityId(nxCommunityOrdersSubEntities);
        mergeMiniappPricing(mapR, communityId, userId, 0, serviceType, -1);
...
//        List<NxCustomerUserCardEntity> cardEntities = nxCustomerUserCardService.queryUserCardByParams(mapC);
    ...
    private void reduceSubOrderRemarkNoQuantity(Integer goodsId, Integer orderUserId, String remark, Integer orderType, Integer spId, Integer deskId) {
        ...
        if (commSubOrderEntity != null) {
            ...
            if (orderQuantity.compareTo(new BigDecimal(1)) == 0) {
                ...
                BigDecimal decimal = new BigDecimal(commSubOrderEntity.getNxCosPrice()).multiply(quantity).setScale(2, BigDecimal.ROUND_HALF_UP);
                ...
                if (commSubOrderEntity.getNxCosHuaxianPrice() != null) {
                    BigDecimal decimal1 = new BigDecimal(commSubOrderEntity.getNxCosHuaxianPrice()).multiply(quantity).setScale(2, BigDecimal.ROUND_HALF_UP);
    ...
    private void reduceSubOrderRemarkQuantity(Integer goodsId, Integer orderUserId,String remark, Integer orderType, Integer spId,Integer deskId) {
        ...
        if (diffZeroSubOrderEntity != null) {
            ...
            if (orderQuantity.compareTo(new BigDecimal(1)) == 0) {
                ...
                BigDecimal decimal = new BigDecimal(diffZeroSubOrderEntity.getNxCosPrice()).multiply(quantity).setScale(2, BigDecimal.ROUND_HALF_UP);
                ...
                if (diffZeroSubOrderEntity.getNxCosHuaxianPrice() != null) {
                    BigDecimal decimal1 = new BigDecimal(diffZeroSubOrderEntity.getNxCosHuaxianPrice()).multiply(quantity).setScale(2, BigDecimal.ROUND_HALF_UP);
            ...
            if (diffSubOrderEntity != null) {
                ...
                if (orderQuantity.compareTo(new BigDecimal(1)) == 0) {
                    ...
                    BigDecimal decimal = new BigDecimal(diffSubOrderEntity.getNxCosPrice()).multiply(quantity).setScale(2, BigDecimal.ROUND_HALF_UP);
                    ...
                    if (diffSubOrderEntity.getNxCosHuaxianPrice() != null) {
                        BigDecimal decimal1 = new BigDecimal(diffSubOrderEntity.getNxCosHuaxianPrice()).multiply(quantity).setScale(2, BigDecimal.ROUND_HALF_UP);
    ...
    private void saveSubOrderRemarkNoQuantity(Integer goodsId, Integer orderUserId, String remark, Integer orderType, Integer spId,
        ...
        if (communityOrdersSubEntityC != null) {
            ...
            BigDecimal subtotal = new BigDecimal(communityOrdersSubEntityC.getNxCosPrice()).multiply(add).setScale(2, BigDecimal.ROUND_HALF_UP);
    ...
    private void saveSubOrderRemarkQuantity(Integer goodsId, Integer orderUserId, String remark, Integer orderType,
        ...
        BigDecimal diffPrice = new BigDecimal(subHuaxianPrice).subtract(new BigDecimal(subPrice)).setScale(2, BigDecimal.ROUND_HALF_UP);
        ...
        if (communityOrdersSubEntityC != null) {
            ...
            if (communityOrdersSubEntity != null ) {
                ...
                if (restQuantity.compareTo(new BigDecimal(0)) == 1) {
                    ...
                    BigDecimal subtotal = new BigDecimal(communityOrdersSubEntity.getNxCosPrice()).multiply(add).setScale(2, BigDecimal.ROUND_HALF_UP);
                    BigDecimal huaxianSubtotal = new BigDecimal(communityOrdersSubEntity.getNxCosHuaxianPrice()).multiply(add).setScale(2, BigDecimal.ROUND_HALF_UP);
                    ...
                    if (communityOrdersSubZero != null) {
                        ...
                        BigDecimal subtotal = new BigDecimal(communityOrdersSubZero.getNxCosPrice()).multiply(add).setScale(2, BigDecimal.ROUND_HALF_UP);
...
//        }
    ...
    public R saveSubOrderRemarkCard(Integer goodsId, Integer orderUserId, Integer deskId, String remark, Integer orderType,
        ...
        BigDecimal differPrice = new BigDecimal(subHuaxianPrice).subtract(new BigDecimal(subPrice)).setScale(2, BigDecimal.ROUND_HALF_UP);
        NxCommunityGoodsEntity goodsEntity = nxCommunityGoodsService.queryObject(goodsId);
        Map<String, Object> map = new HashMap<>();
        ...
        if (userCardEntitys != null) {
            ...
            if (userCardEntitys.getNxCucaIsSelected() == 1) {
                ...
                if (communityOrdersSubEntity != null && goodsEntity.getNxCgGoodsHuaxianPrice() != null) {
                    ...
                    if (count > 0) {
                        ...
                        if (restQuantity.compareTo(new BigDecimal(0)) == 1) {
                            BigDecimal add = orderQuantity.add(new BigDecimal(1));
                            BigDecimal subtotal = new BigDecimal(communityOrdersSubEntity.getNxCosPrice()).multiply(add).setScale(2, BigDecimal.ROUND_HALF_UP);
                            BigDecimal huaxianSubtotal = new BigDecimal(subHuaxianPrice).multiply(add).setScale(2, BigDecimal.ROUND_HALF_UP);
                            communityOrdersSubEntity.setNxCosQuantity(add.toString());
                            communityOrdersSubEntity.setNxCosSubtotal(subtotal.toString());
                            communityOrdersSubEntity.setNxCosHuaxianSubtotal(huaxianSubtotal.toString());
                            nxCommunityOrdersSubService.update(communityOrdersSubEntity);
                ...
                if (subOrderEntity != null) {
                    ...
                    BigDecimal add = orderQuantity.add(new BigDecimal(1));
                    BigDecimal subtotal = new BigDecimal(subPrice).multiply(add).setScale(2, BigDecimal.ROUND_HALF_UP);
...
//        map.put("type", orderType);
    ...
    public R addSubOrder(Integer goodsId, Integer orderUserId, Integer deskId, Integer orderType, Integer spId, Integer pindanId,Integer serviceType) {
        ...
        NxCommunityGoodsEntity goodsEntity = nxCommunityGoodsService.queryObject(goodsId);
        serviceType = resolveCartServiceType(goodsEntity, serviceType);

        if (!hasHuaxianPrice(goodsEntity)) {
            //commAdd
            NxCommunityOrdersSubEntity subOrderEntity = findCartSubOrderForAdd(
                    orderUserId, deskId, goodsId, orderType, spId, serviceType, null, null);
            if (subOrderEntity != null) {
                BigDecimal orderQuantity = new BigDecimal(subOrderEntity.getNxCosQuantity());
                BigDecimal add = orderQuantity.add(new BigDecimal(1));
                BigDecimal subtotal = new BigDecimal(subOrderEntity.getNxCosPrice()).multiply(add).setScale(2, BigDecimal.ROUND_HALF_UP);
                subOrderEntity.setNxCosQuantity(add.toString());
                subOrderEntity.setNxCosSubtotal(subtotal.toString());
                subOrderEntity.setNxCosServiceDate(formatWhatDay(0));
                subOrderEntity.setNxCosServiceType(serviceType);
                nxCommunityOrdersSubService.update(subOrderEntity);
            } else {
                saveFirstSubOrderInternal(goodsEntity, orderUserId, deskId, spId, orderType, pindanId, serviceType);
            }

        } else {
            //pandan
            //先查是否有普通订单
            NxCommunityOrdersSubEntity putongSubOrder = findCartSubOrderForAdd(
                    orderUserId, deskId, goodsId, orderType, spId, serviceType, 0, null);
            if (putongSubOrder != null) {
                //普通订单加 1
                BigDecimal orderQuantity = new BigDecimal(putongSubOrder.getNxCosQuantity());
                BigDecimal add = orderQuantity.add(new BigDecimal(1));
                BigDecimal subtotal = new BigDecimal(putongSubOrder.getNxCosPrice()).multiply(add).setScale(2, BigDecimal.ROUND_HALF_UP);
                ...
                putongSubOrder.setNxCosServiceType(serviceType);
                nxCommunityOrdersSubService.update(putongSubOrder);

            } else {
                //查询优惠订单
                NxCommunityOrdersSubEntity communityOrdersSubEntity = findCartSubOrderForAdd(
                        orderUserId, deskId, goodsId, orderType, spId, serviceType, null, 0);
                if (communityOrdersSubEntity != null && hasHuaxianPrice(goodsEntity)) {
        ...
        //giveapply
        Map<String, Object> mapA = buildOpenCartQueryMap(orderUserId, deskId, orderType, spId, serviceType);
        List<NxCommunityOrdersSubEntity> nxCommunityOrdersSubEntities = nxCommunityOrdersSubService.querySubOrdersByParams(mapA);

        R response = R.ok().put("data", nxCommunityOrdersSubEntities);
        return mergeMiniappPricingResponse(response, goodsEntity.getNxCgCommunityId(), orderUserId,
                orderType, serviceType, spId);
    ...
    public R addSubOrderReduce(Integer goodsId, Integer orderUserId, Integer orderId, Integer deskId,   Integer number) {
        ...
        BigDecimal decimal = new BigDecimal(ordersEntity.getNxCoTotal()).subtract(multiply).setScale(2, BigDecimal.ROUND_HALF_UP);
...
//    }
    ...
    public R reduceSubOrder(Integer goodsId, Integer orderUserId, Integer orderType, Integer spId,
        ...
        mergeMiniappPricing(mapData, resolveCommunityId(nxCommunityOrdersSubEntities), orderUserId,
                orderType, serviceType, spId);
    ...
    /**
     * 购物车车道与商品 nxCgServiceType 对齐：外卖商品进外卖购物车。
     */
    private Integer resolveCartServiceType(NxCommunityGoodsEntity goodsEntity, Integer serviceType) {
        if (goodsEntity == null || goodsEntity.getNxCgServiceType() == null) {
            return serviceType != null ? serviceType : 0;
        }
        Integer goodsServiceType = goodsEntity.getNxCgServiceType();
        if (serviceType == null || !goodsServiceType.equals(serviceType)) {
            return goodsServiceType;
        }
        return serviceType;
    }

    private boolean hasHuaxianPrice(NxCommunityGoodsEntity goodsEntity) {
        if (goodsEntity == null) {
            return false;
        }
        String huaxianPrice = goodsEntity.getNxCgGoodsHuaxianPrice();
        return huaxianPrice != null && !huaxianPrice.trim().isEmpty();
    }

    private Map<String, Object> buildOpenCartQueryMap(Integer orderUserId, Integer deskId,
                                                      Integer orderType, Integer spId, Integer serviceType) {
        Map<String, Object> map = new HashMap<>();
        map.put("orderUserId", orderUserId);
        map.put("deskId", deskId);
        map.put("status", -1);
    ...
    /**
     * 按车道查购物车行；若历史数据车道错误则自动纠正 nxCosServiceType。
     */
    private NxCommunityOrdersSubEntity findCartSubOrderForAdd(Integer orderUserId, Integer deskId, Integer goodsId,
                                                              Integer orderType, Integer spId, Integer serviceType,
                                                              Integer diffPrice, Integer dayuDiffPrice) {
        Map<String, Object> map = new HashMap<>();
        map.put("orderUserId", orderUserId);
        map.put("deskId", deskId);
        map.put("goodsId", goodsId);
        ...
        if (dayuDiffPrice != null) {
            map.put("dayuDiffPrice", dayuDiffPrice);
        }
        NxCommunityOrdersSubEntity found = nxCommunityOrdersSubService.queryChangeSubOrderByParams(map);
        if (found != null) {
            return found;
        }
        map.remove("serviceType");
        found = nxCommunityOrdersSubService.queryChangeSubOrderByParams(map);
        if (found == null) {
            return null;
        }
        if (serviceType != null && !serviceType.equals(found.getNxCosServiceType())) {
            found.setNxCosServiceType(serviceType);
            nxCommunityOrdersSubService.update(found);
        }
        return found;
    }

    private NxCommunityOrdersSubEntity saveFirstSubOrderInternal(NxCommunityGoodsEntity goodsEntity,
                                                                 Integer orderUserId, Integer deskId, Integer spId,
                                                                 Integer orderType, Integer pindanId,
                                                                 Integer serviceType) {
        NxCommunityOrdersSubEntity subEntity = new NxCommunityOrdersSubEntity();
        subEntity.setNxCosCommunityId(goodsEntity.getNxCgCommunityId());
        Integer communityId = goodsEntity.getNxCgCommunityId();
                ...
                if (totalAmount.compareTo(new BigDecimal(goodsEntity.getNxCgPromotionAmount())) > -1) {
                    ...
                    for (NxCommunityOrdersSubEntity ordersSubEntity : subEntities) {
                        ...
                        if (nxCosQuantity.compareTo(new BigDecimal(1)) == 0) {
                            ...
                            BigDecimal multiply = restQuantity.multiply(new BigDecimal(goodsEntity.getNxCgGoodsPrice()))
    ...
    public R saveFirstSubOrder(Integer goodsId, Integer orderUserId, Integer deskId, Integer spId, Integer orderType, Integer pindanId, Integer serviceType) {
        ...
        NxCommunityGoodsEntity goodsEntity = nxCommunityGoodsService.queryObject(goodsId);
        serviceType = resolveCartServiceType(goodsEntity, serviceType);
        NxCommunityOrdersSubEntity subEntity = saveFirstSubOrderInternal(
                goodsEntity, orderUserId, deskId, spId, orderType, pindanId, serviceType);

        R response = R.ok().put("data", subEntity);
        return mergeMiniappPricingResponse(response, goodsEntity.getNxCgCommunityId(), orderUserId,
                orderType, serviceType, spId);
...
//    }
    ...
    public R deleteSubOrders(@RequestBody Integer[] nxOrdersSubIds, Integer orderType,
                             Integer serviceType, Integer spId) {
        Integer userId = 0;
        ...
        for (Integer i : nxOrdersSubIds) {
            ...
            if (communityId == null) {
                communityId = subEntity.getNxCosCommunityId();
            }
        }
        ...
        R response = R.ok();
        return mergeMiniappPricingResponse(response, communityId, userId,
                orderType == null ? 0 : orderType, serviceType, spId);
    }
    ...
    private void mergeMiniappPricing(Map<String, Object> target, Integer communityId, Integer userId,
                                     Integer orderType, Integer serviceType, Integer spId) {
        if (communityId == null || userId == null || userId <= 0) {
            CartPricingSupport.mergeIntoMap(target, CartPricingSupport.emptyPricing());
            return;
        }
        MiniappCartPricingHelper.mergeIntoMap(target, cartPricingService, communityId, userId,
                orderType, serviceType, spId);
    }

    private R mergeMiniappPricingResponse(R response, Integer communityId, Integer userId,
                                          Integer orderType, Integer serviceType, Integer spId) {
        if (communityId == null || userId == null || userId <= 0) {
            return CartPricingSupport.mergeIntoResponse(response, CartPricingSupport.emptyPricing());
        }
        return MiniappCartPricingHelper.mergeIntoResponse(response, cartPricingService, communityId, userId,
                orderType, serviceType, spId);
    }

    private Integer resolveCommunityId(List<NxCommunityOrdersSubEntity> subs) {
        if (subs == null || subs.isEmpty()) {
            return null;
        }
        for (NxCommunityOrdersSubEntity sub : subs) {
            if (sub.getNxCosCommunityId() != null) {
                return sub.getNxCosCommunityId();
            }
        }
        return null;
    }
