...
public interface NxCommunityOrdersSubService {
    ...
    CartSnapshot buildCartSnapshot(Integer communityId, Integer userId);

    CartSnapshot buildCartSnapshot(Integer communityId, Integer userId,
                                   Integer orderType, Integer serviceType, Integer spId);

    /** 从已保存订单子单构建快照（POS 用券 / 订单定价）。 */
    CartSnapshot buildCartSnapshotFromOrder(Integer communityId, Integer customerUserId, Integer orderId);
}
